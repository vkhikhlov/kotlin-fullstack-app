    ...
    override fun Template.render() = root {
        ...
        fun VDOMBuilder<DIV>.verticalMenu(direction: VerticalDirection) {
            fun VDOMBuilder<DIV>.menu() = div("ui attached fluid secondary pointing tabular menu") {
                v.bind.style { if (direction == VerticalDirection.BOTTOM) transform = "rotate(-180deg)" }
                menuItems {
                    a(classes = "item") {
...
fun VDOMBuilder<DIV>.tabPanel(props: TabPanelProps.() -> Unit = { }) = child(TabPanelOptions, props)